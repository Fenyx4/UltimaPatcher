...
float      a =  2.000;                // saturation 
float      b =  1.000;                // brightness
float      c1=  2.150;                // contrast 1 funct param.
...
{
  float3 c  = tex2D(s_w, VAR.CT).xyz;
  float3 o1 = tex2D(s_w, VAR.UL).xyz;
  float3 o2 = tex2D(s_w, VAR.UR).xyz;
  float3 o3 = tex2D(s_w, VAR.DR).xyz;
  float3 o4 = tex2D(s_w, VAR.DL).xyz;
  float3 i1 = tex2D(s_w, VAR.iUL).xyz;
  float3 i2 = tex2D(s_w, VAR.iUR).xyz;
  float3 i3 = tex2D(s_w, VAR.iDD.zw).xyz;
  float3 i4 = tex2D(s_w, VAR.iDD.xy).xyz;
...
{
  float3 color = tex2D(s_p, VAR.CT).xyz;
