...
float4 S_FRAGMENT0 ( in VERTEX_STUFF_W VAR ) : COLOR
{
	...
	half2 dx = half2(2*ps.x, 0.0); 
	half2 dy = half2( 0.0,2*ps.y);
...
float4 S_FRAGMENT ( in VERTEX_STUFF_W VAR ) : COLOR
{
	...
	half3 B = tex2D(s_w,d11-dy).xyz;
	half3 D = tex2D(s_w,d11-dx).xyz;
	half3 E = tex2D(s_w,d11   ).xyz;
	half3 F = tex2D(s_w,d11+dx).xyz;
	half3 H = tex2D(s_w,d11+dy).xyz;
